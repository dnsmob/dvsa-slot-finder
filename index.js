...
const wsChromeEndpointurl = 'ws://127.0.0.1:9222/devtools/browser/69e122a1-fb1f-4c5a-a590-3b0a50f721ad'
...
const test = async () => {
    ...
    await page.type('input#driving-licence-number', 'DOSSA709287D99FV')
    await page.type('input#application-reference-number', '49517258')

    await Promise.all([
        page.click('input#booking-login'),
        page.waitForNavigation({ waitUntil: 'networkidle2' })
    ]);

    await Promise.all([
        page.click('a#date-time-change'),
        page.waitForNavigation({ waitUntil: 'networkidle2' })
    ]);

    await page.click('input#test-choice-earliest')

    await Promise.all([
        page.click('input#driving-licence-submit'),
        page.waitForNavigation({ waitUntil: 'networkidle2' })
    ]);

    await Promise.all([
        page.click('a#change-test-centre'),
        page.waitForNavigation({ waitUntil: 'networkidle2' })
    ]);

    await page.click('input#test-centres-input', { clickCount: 3 })
    await page.type('input#test-centres-input', 'SE10')

    await Promise.all([
        page.click('input#test-centres-submit'),
        page.waitForNavigation({ waitUntil: 'networkidle2' })
    ]);
